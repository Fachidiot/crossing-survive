...
public class Destructible : MonoBehaviour
{
    ...
    public float hp_regen = 0f; // 회복력

    [Header("Targeting")]
    public AttackGroup attack_group;    // 공격대상 지정
    public GroupData team_group;        // 아군 지정
    public GroupData required_item;     // 공격에 필요한 아이템 (플레이어 전용)
    public bool attack_melee_only = true;   // 원거리 무기 적용 제한
    public float hit_range = 1f;        // 공격 가능 범위

    [Header("Loot")]
    public int xp = 0;          // 경험치
    public string xp_type;      // 경험치 종류
    public SData[] loots;       // 드랍 아이템 리스트

    [Header("FX")]
    public bool shake_on_hit = true;    // 피격시 흔들림
    public float destroy_delay = 0f;    // 시체 사라지는 시간 조정
    ...
    [Header("UI")]
    public GameObject hp_bar;   // hp바
    public GameObject hit_fx;   // 피격 이펙트
    public GameObject death_fx; // 사망 이펙트

    [Header("SFX")]
    public AudioClip hit_sound;     // 피격 사운드
    public AudioClip death_sound;   //사망 사운드
